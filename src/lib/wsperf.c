...
uint64_t get_cycle_count(void) {
  ...
  // unfortunately, CLOCK_PROCESS_CPUTIME_ID is not defined
  // for FreeBSD, so we explicitly define it to the best of
  // our knowledge
...
int init_wsperf(void) {
    ...
    for (uint32_t i=0; i<work_size; i++) {
        kidcycle[i] = (uint64_t *)calloc(
            max_kids, sizeof(uint64_t));
        if (!kidcycle[i]) {
            error_print("failed init_wsperf calloc of kidcycle[i]");
            return 0;
        }
        kidcount[i] = (uint64_t *)calloc(
            max_kids, sizeof(uint64_t));
        if (!kidcount[i]) {
            error_print("failed init_wsperf calloc of kidcount[i]");
            return 0;
        }
        flushcount[i] = (uint64_t *)calloc(
            max_kids, sizeof(uint64_t));
        if (!flushcount[i]) {
...
int init_wsperf_kid_name(uint32_t uid, char *name) {
    ...
    uint32_t cpsz = sizeof(*name);
    num_kids = uid;
    uint32_t i;

    // NOTE:  memory added by realloc is uninitialized by
    // the system, so we need to initialize it

    // Expand kidname if necessary
    if (uid >= max_kids) {
        kidname = (char **)realloc(
            kidname, (max_kids+10)*sizeof(char *));
        if (!kidname) {
            error_print("failed init_wsperf_kid_name realloc of kidname");
            return 0;
        }
        for (i=max_kids; i<max_kids+10; i++) {
            kidname[i] = (char *)calloc(
                MAX_CHARS, sizeof(char));
            if (!kidname[i]) {
...
int print_wsperf(mimo_t * mimo) {
    ...
    double finaltime, kidtime = 0.0, time0;
    uint64_t wstotal;
    int i;
    uint32_t * kidindx;
    double * kidtot;
