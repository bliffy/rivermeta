...
static int wsprockeystate_process_key(
     void*,
     wsdata_t*,
     ws_doutput_t*,
     int);
static int wsprockeystate_process_keyvalue(
     void*,
     wsdata_t*,
     ws_doutput_t*,
     int);
static int wsprockeystate_delete_key(
     void*,
     wsdata_t*,
     ws_doutput_t*,
     int);
static int wsprockeystate_process_flush(
     void*,
     wsdata_t*,
     ws_doutput_t*,
     int);
...
static void wsprockeystate_expire(
          void* vstate,
          void* vinstance)
{
     wsprockeystate_inst_t* proc =
          (wsprockeystate_inst_t*)vinstance;
     proc->kid->expire_func(
          proc->kproc,
          vstate,
          proc->dout,
          proc->outtype_tuple);
...
static int wsprockeystate_cmd_options(
          int argc,
          char* const* argv, 
          wsprockeystate_inst_t* proc,
          void* type_table)
{
     ...
     while ((op=getopt(argc, argv, proc->option_str)) != EOF)
     {
          ...
          case 'J':
               ...
               break;
          case 'V':
               proc->label_value = wssearch_label(
                    type_table, optarg);
               break;
          case 'M':
     ...
     while (optind < argc)
     {
          proc->label_key = wssearch_label(
               type_table, argv[optind]);
          tool_print(
               "%s using key with label %s",
...
// the following is a function to take in command arguments
// and initalize this processor's instance..
//  also register as a source here..
// return 1 if ok
// return 0 if fail
int wsprockeystate_init(
          int argc,
          char* const* argv,
          void** vinstance,
          void* type_table,
          wsprockeystate_kid_t* kid)
{
     // allocate instance of this processor
     wsprockeystate_inst_t* proc = (wsprockeystate_inst_t*)calloc(1, sizeof(wsprockeystate_inst_t));
     if (!proc) {
     ...
     *vinstance = proc;
     ...
     if (kid->labeloffset) {
          wsinit_labeloffset(
               kid->labeloffset, proc->kproc, type_table);
     }
     ...
     if (proc->sharelabel) {
          if (kid->expire_func) {
               ...
               sh5_sh_opts->sh_callback =
                    wsprockeystate_expire;
               sh5_sh_opts->proc = proc; 
               if (!stringhash5_create_shared_sht(
                         type_table,
                         (void**) &proc->state_table,
                         proc->sharelabel,
                         proc->buflen,
                         kid->state_len,
                         NULL,
          ...
          proc->state_table = stringhash5_create(
               0, proc->buflen, kid->state_len);
          if (kid->expire_func)
          {
               stringhash5_set_callback(
                    proc->state_table,
                    wsprockeystate_expire,
                    proc);
          }
     ...
     // use the stringhash5-adjusted value of max_records
     // to reset buflen
...
// this function needs to decide on processing function
// based on datatype given.. also set output types as
// needed (unless a sink)
// return 1 if ok
// return 0 if problem
proc_process_t wsprockeystate_input_set(
          void* vinstance,
          ...
          void* type_table)
{
     ...
     if (!proc->outtype_tuple) {
          proc->outtype_tuple = ws_add_outtype(
               olist, dtype_tuple, NULL);
     }
     ...
     {
          return wsprockeystate_delete_key;
     }

     if (proc->label_value)
          return wsprockeystate_process_keyvalue;
     else
          return wsprockeystate_process_key;
}

// proc processing function assigned to a specific data
// type in proc_io_init
// return 1 if output is available
// return 2 if not output

// Commented out code represents in-place copy, but uses
// more memory -
// The current version deflates to a persistent buffer and
// copies out (more computation)
          ...
          int type_index)
{
     ...
     int found;
     void* sdata;
     for (int j=0; j<mset_len; j++) {
          found = 0;
          sdata = stringhash5_find_attach_wsdata(
               proc->state_table, mset[j]);
          if (sdata)
               found = 1;
          rtn += proc->kid->update_func(
               proc->kproc,
               sdata,
               input_data,
               mset[j]);
          ...
          int type_index)
{
     ...
     int found = 0;
     void* sdata = stringhash5_find_attach_wsdata(
          proc->state_table, key);
     if (sdata)
          found = 1;
              ...
              &mset))
     {
          for (j = 0; j < mset_len; j++ ) {
               rtn += proc->kid->update_value_func(
                    proc->kproc,
                    sdata,
                    input_data,
                    key,
