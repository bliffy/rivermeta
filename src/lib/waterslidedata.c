...
void wsdatatype_profile_verbose(
        void * data,
        void * ignore)
{
    ...
    if (allocated) {
        ...
        status_print(
            "dtype %s: allocd %u recovered %u %s",
             dtype->name,
             allocated, 
             buffered, 
    ...
    if (allocated_ptr) {
        uint32_t buffered_ptr = wsfree_list_size(
            dtype->freeq_ptr);
        status_print(
            "dtype %s ptr: allocd %u recovered %u %s",
            dtype->name,
            allocated_ptr, 
            buffered_ptr, 
...
void wsdatatype_profile(
        void * data,
        void *ignore)
{
    wsdatatype_t * dtype = (wsdatatype_t *)data;
    uint32_t allocated = wsfree_list_allocated(dtype->freeq);
    uint32_t allocated_ptr = wsfree_list_allocated(
        dtype->freeq_ptr);
    int flag_error = 0;
    ...
    if (allocated || allocated_ptr) {
        ...
        uint32_t buffered_ptr = wsfree_list_size(
            dtype->freeq_ptr);
        if ( (allocated != buffered)
             || (allocated_ptr != buffered_ptr))
        {
            if (!flag_error) {
...
static inline void free_dtype_freeq_data(
        wsfree_list_t * fl,
        int hasdata)
{
#ifndef WS_PTHREADS
...
static inline void free_dtype_hugeblock_freeq_data(
        wsfree_list_t * fl,
        int hasdata)
{
#ifndef WS_PTHREADS
...
void wsdatatype_free_module_names(
        void * data,
        void * ignore)
{
        ...
        uint64_t hash)
{
    ...
    mimo_datalists_t * mdl = 
        (mimo_datalists_t *)v_type_table;
    label_alias = (wslabel_t*)listhash_find_attach_reference(
        mdl->label_table,
        (const char *)&hash,
        sizeof(uint64_t),
        parent);
...
static inline wslabel_t * wsregister_label_internal(
        void * v_type_table,
        const char * name)
{
    ...
    wslabel_t * label;
    //look up label..
    label = (wslabel_t*)listhash_find_attach(
        mdl->label_table,
        name,
        namelen);
...
static inline wslabel_t * wsregister_label_internal_len(
        void * v_type_table,
        const char * name,
        int len)
{
    ...
    if (!label->name) {
        ...
        label->hash = evahash64(
            (uint8_t*)name, len, 0x22341133);
        wsregister_label_hash(
            v_type_table, label, label->hash);
    }

    return label;
}

static inline void wsset_label_index(
        void * v_type_table,
        wslabel_t * label)
{
...
wslabel_t * wsregister_label_len(
        void * v_type_table,
        const char * name,
        int len)
{
...
static int wsdatatype_default_to_uint64(
        wsdata_t * wsd,
        uint64_t * u)
{ return 0; }

static int wsdatatype_default_to_uint32(
        wsdata_t * wsd,
        uint32_t* u)
{ return 0; }

static int wsdatatype_default_to_int64(
        wsdata_t * wsd,
        int64_t* i)
{ return 0; }

static int wsdatatype_default_to_int32(
        wsdata_t * wsd,
        int32_t* i)
{ return 0; }

static int wsdatatype_default_to_double(
        wsdata_t * wsd,
        double* d)
{ return 0; }
...
//create data structure to store data types.. based on
//stringname
...
wsdatatype_t * wsdatatype_register(
        void * v_type_table,
        const char * name,
        int len,
        wsdatatype_hash hf,
        ...
        wsdatatype_serialize serialf)
{
    ...
    dtype = (wsdatatype_t*)listhash_find(
        mdl->dtype_table, name, namelen);
    if (dtype) {
        fprintf(stderr,"data type %s already registered, ignoring new registry", name);
        return dtype;
    }
    dtype = (wsdatatype_t*)listhash_find_attach(
        mdl->dtype_table, name, namelen);

    dtype->len = len;
    dtype->name = strdup(name);
    dtype->namehash = evahash64(
        (uint8_t*)name,
        namelen,
        WSDATA_HASH_SEED);
    ...
    dtype->freeq_ptr = wsfree_list_init(
        0, wsdata_ptr_allocator, dtype);
    dtype->freeq = wsfree_list_init(
        0, wsdata_allocator, dtype);
...
wsdatatype_t * wsdatatype_register_generic(
        void * type_table,
        const char * name,
        int len)
{
    return wsdatatype_register(
        type_table,
        name, len,
...
wsdatatype_t * wsdatatype_register_generic_ts(
        void * type_table,
        const char * name,
        int len)
{
...
wsdatatype_t * wsdatatype_register_generic_pf(
        void * type_table,
        const char * name,
        int len,
        wsdatatype_print pf,
...
wsdatatype_t * wsdatatype_register_generic_pf_ts(
        void * type_table,
        const char * name,
        int len,
        wsdatatype_print pf,
...
//return 0 on fail
int wsdatatype_register_alias(
        void * v_type_table,
        wsdatatype_t * wsdtype,
        const char * name)
{
    mimo_datalists_t * mdl = (mimo_datalists_t *)v_type_table;
    wsdatatype_t * dtype;
    ...
    dtype = (wsdatatype_t*)listhash_find_attach_reference(
        mdl->dtype_table,
        name,
        namelen,
        wsdtype);
...
int wsdatatype_match(
        void * v_type_table,
        wsdatatype_t * wsdtype,
        const char * name)
{
    // until we get a fancier equivilence test..
    // check lookup
...
int wslabel_match(
        void * v_type_table,
        const wslabel_t * label,
        const char * name)
{
    // until we get a fancier equivilence test..
    // check lookup
    wslabel_t * lookup = wsregister_label(
        v_type_table, name);
    // check if pointer match
    if (lookup == label)
        return 1;
...
// default hash function for generic non-pointer structure
// data
        ...
        ws_printtype_t type)
{
    memcpy((void * __restrict__)buf,
          (void * __restrict__)data->data,
          data->dtype->len);
    return data->dtype->len;
}

void wsdatatype_default_init(
        wsdata_t * wsdata,
        wsdatatype_t * dtype)
{
    if (dtype->len)
...
wsdata_t * wsdatatype_default_copy(wsdata_t * wsdata) {
    ...
    // TODO: Should do recursive call to
    // wsdatatype_default_copy for most cases. Some types
    // need special handling, such as string. That would
    // leave the new string buffer pointing to the old
    // string's buffer. For now, changes to the old string
    // buffer will change the new string buffer.
    // In other words, a call like:
    // wsdata_t * newdep = wsdatatype_default_copy(dep),
    // then followed by: wsdata_assign_dependency(newdep,
    // newdata).
...
uint8_t * wsdatatype_default_serialize(
        wsdata_t * wsdata,
        int * len)
{
...
static int wslabel_nested_recurse(
        void * type_table,
...
int wslabel_nested_search_build(
        ...
        const char * search)
{
    return wslabel_nested_recurse(
        type_table, nest, search, 0);
}

static int wslabel_nested_recurse_ext(
        void * type_table,
...
int wslabel_nested_search_build_ext(
        ...
        int uid)
{
    return wslabel_nested_recurse_ext(
        type_table,
        nest,
        search,
        0,
