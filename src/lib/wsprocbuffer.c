...
static int wsprocbuffer_process_label(
    void*, wsdata_t*, ws_doutput_t*, int);
static int wsprocbuffer_process_unlabeled(
    void*, wsdata_t*, ws_doutput_t*, int);
static int wsprocbuffer_process_el_label(
    void*, wsdata_t*, ws_doutput_t*, int);
static int wsprocbuffer_process_el_unlabeled(
    void*, wsdata_t*, ws_doutput_t*, int);


static int wsprocbuffer_cmd_options(
        int argc,
        char* const* argv, 
        ...
        wsprocbuffer_kid_t* kid)
{
    ...
    wsprocbuffer_inst_t* proc = (wsprocbuffer_inst_t*)calloc(
        1, sizeof(wsprocbuffer_inst_t));
    if (!proc) {
...
// this function needs to decide on processing function based
// on datatype given..
// also set output types as needed (unless a sink)
// return 1 if ok
// return 0 if problem
        ...
        void* type_table)
{
    ...
    if (input_type == dtype_tuple) {
        ...
        if (proc->kid->element_func) {
            if (proc->nest.cnt)
                return wsprocbuffer_process_el_label;
            return wsprocbuffer_process_el_unlabeled;
        }
        else if (proc->kid->decode_func) {
            if (proc->nest.cnt)
                return wsprocbuffer_process_label;
            return wsprocbuffer_process_unlabeled;
        }
    }
    return NULL;
}

static inline int wsprocbuffer_nested_search(
        wsprocbuffer_inst_t* proc,
        wsdata_t* tdata,
        int sid,
        int* found)
            ...
            if (lset->id[i]) {
                for (j=0; j<mset_len; j++) {
                    ...
                    if (mset[j]->dtype == dtype_tuple) {
                        cnt += wsprocbuffer_nested_search(
                            proc, mset[j],
                            lset->id[i],
                            found);
                    }
...
// proc processing function assigned to a specific data type
// in proc_io_init
        ...
        int type_index)
{
    ...
    int found = 0;
    if (wsprocbuffer_nested_search(proc,input_data,0,&found) || (!found && proc->kid->pass_not_found))
    {
        proc->pass++;
...
static inline int wsprocbuffer_nested_el_search(
        wsprocbuffer_inst_t* proc,
        wsdata_t* tdata,
        int sid,
        int* found)
                             ...
                             &mset))
        {
            ...
            if (lset->id[i]) {
                for (j = 0; j < mset_len; j++ ) {
                    ...
                    // search for sublabels
                    if (mset[j]->dtype == dtype_tuple) {
                        cnt += wsprocbuffer_nested_el_search(
                            proc,
                            mset[j],
                            lset->id[i],
                            found);
        ...
        int type_index)
{
    ...
    proc->recv++;
    int found = 0;
    if (wsprocbuffer_nested_el_search(proc, input_data, 0, &found) || (!found && proc->kid->pass_not_found))
    {
        proc->pass++;
        ws_set_outdata(input_data, proc->outtype_tuple, dout);
    }
        ...
        int type_index)
{
    ...
    for (int i=0; i<tlen; i++) {
        member = tuple->member[i];
        rtn += proc->kid->element_func(
            proc->kproc, input_data, member);
    }
    if (!rtn)
        return 1;
