// main execution thread for pipeline.. polls all sources
...
// Globals
//  next thread to do local+external
uint32_t hop_to_next_thread_locext;
          ...
          nhqueue_t* jobq_freeq)
{
     ...
     while ((job = queue_remove(jobq)) != NULL)
     {
          cnt++;
          // call processor
          // each job is dequeued from the appropriate
          // thread's jobq

          //walk list of subscribers
          for (sub = job->subscribers; sub; sub = sub->next)
          {
               if ((!sub->src_label || wsdata_check_label(
                         job->data, sub->src_label)))
               {
                    dprint("running data %s thru %s",
                           job->data->dtype->name,
...
int ws_execute_graph(mimo_t* mimo) {
     ...
     // needed to know who should check for valid source
     ...
     if (deadlock_firehose_shutoff)
     {
          ...
          {
               // try emptying all external jobs, then 
               // jobs in failover queue, before turning
               // back on the firehose (i.e., proc_sources
               // or input source to be executed)

               int i, allclear;
               // attempt to empty out all failoverq before
               // proceeding
               if(fqueue_remove(mimo->failoverq[nrank], (void**)&data, (void**)&scursor))
               {
                    ...
                    {
                         // add data back to head NOT tail to
                         // preserve metadata order in queue
                         fqueue_add_front(
                              mimo->failoverq[nrank],
                              data,
                              scursor);
                    }

                    WS_MUTEX_LOCK(&(mimo->lock));
                    // check all failover queues and restore
                    // normal processing if all queues are
                    // empty
                    allclear = 1;
                    ...
                    dprint("allclear = %d - %s\n",
                           allclear,
                           (allclear == 0
                            ? "leave input sources off (still recovering from deadlock)"
                            : "re-enable input sources (deadlock gone)"));
#endif
                    if (allclear) {
          ...
          cursor = cursor->next)
     {
          ...
          if (deadlock_firehose_shutoff)
          {
               // a deadlock-induced temporary shutoff of all
               // input sources has occured
               if (!src_out) {
                    // fake that a metadata was emitted to
                    // avoid exiting the WS graph
                    src_out = 1;
               }
               break;
          }
          if (cursor->pinstance->thread_id != nrank)
               continue;
#endif // WS_PTHREADS
          if (cursor->proc_func) {
               // alloc a data type
...
#endif // WS_PTHREADS
     {
          ...
          jcnt += ws_do_local_jobs(
               mimo,
               mimo->jobq[nrank],
               mimo->jobq_freeq[nrank]);
          // pop and handle external (shared) jobq until empty
          jcnt += WS_DO_EXTERNAL_JOBS(
               mimo, mimo->shared_jobq[nrank]);
...
// of local and external queue until there's no more
// jobs left in queue... then return 0
int ws_execute_exiting_graph(mimo_t* mimo)
{
     ...
     if (mimo->mgc) {
          ...
          // attempt to empty out all failoverq
          if (fqueue_remove(
                  mimo->failoverq[nrank],
                  (void**)&data,
                  (void**)&scursor))
     ...
     // pop and handle local jobq until empty...
     jobs_cnt += ws_do_local_jobs(
          mimo, mimo->jobq[nrank], mimo->jobq_freeq[nrank]);

     //pop and handle external (shared) jobq until empty...
     jobs_cnt += WS_DO_EXTERNAL_JOBS(
          mimo, mimo->shared_jobq[nrank]);
...
int mimo_flush_graph(mimo_t* mimo)
{
     ...
     wsdata_t* wsd_flush = wsdata_alloc(
          mimo->flush.outtype_flush.dtype);
     if (!wsd_flush)
          return 0;
     ...
     for (i=0; i<mimo->flush.cnt; i++)
     {
          ...
          // we do have the possibility of metadata in
          // previous flushers' proc's (e.g., proc_flush)
...
void ws_destroy_graph(mimo_t* mimo)
{
     ...
     // This is serialized so that output is not scrambled
     // by multiple threads that are simultaneously reporting
     // their kids' statistics.
          ...
          cursor = cursor->next)
     {
          ...
          if( 0 == nrank
              && cursor->output_type_list.outtype_q )
          {
