...
int check_kid_documentation(
    ...
    if ((sh_file_handle = DLOPEN(spath))) {
        ...
        // verify that this variable exists and has a
        // positive length 
        char * proc_description = (char *)DLSYM(
        ...
        // verify that this variable exists and has at
        // least one entry 
        proc_example_t * examples = (proc_example_t*)DLSYM(
        ...
        proc_option_t * opt = (proc_option_t*)DLSYM(
        ...
        // verify that this variable exists 
        proc_port_t * ports = (proc_port_t*)DLSYM(
...
int print_module_help_search(
     ...
     if ((sh_file_handle = DLOPEN(spath))) {
          ...
          char * ns_opt = (char*)DLSYM(
              sh_file_handle,"proc_nonswitch_opts");
          char ** itypes = (char**)DLSYM(
              sh_file_handle,"proc_input_types");
          char ** otypes = (char**)DLSYM(
              sh_file_handle,"proc_output_types");
...
void load_proc_dir_term(const char * dirname, char * term) {
     int n;
     //char spath[5000];
