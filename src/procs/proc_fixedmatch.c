     ...
     while (fgets(line, 2000, fp)) {
		...
		linep = line;
          ...
          if (linep[0] == '"') {
			...
			tool_print("matching \"%.*s\"", (int)matchlen, matchstr);
...
static inline int find_fixedmatch(proc_instance_t * proc, wsdata_t * wsd, const char * content,
                                  size_t len, wsdata_t * tdata, wsdata_t * tparent) {
     if (len == 0) {
          return 0;
     }
