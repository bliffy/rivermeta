...
// a graph used to implement Tarjan's algorithm for detecting
// strongly connected components of a directed graph, which
// gives conditions for the existence of cycles, which, in
// our case, can cause a cyclical wait on shared queues, a
// necessary condition for the existence of communication
// deadlocks in WS's directed graph
typedef struct _tarjan_graph_t_ {
    nhqueue_t** digraph; // the directed graph
    nhqueue_t* scc_list; // all strongly connected components
    wsstack_t* stack;
    int num_nodes;    // number of vertices
    int num_edges;    // number of edges
    int num_scc;   // cardinality of the set of strongly
                   // connected components
    int preCount;     // preorder number counter
    int* lowlink;     // low number of v
    uint8_t* visited; // to check if a node, v, is visited
} tarjan_graph_t;


static inline void dfs(tarjan_graph_t*, int);
static inline void show_scc(tarjan_graph_t*);

static inline tarjan_graph_t* tarjan_graph_init(
        int num_nodes)
{
    if (num_nodes <= 0) {
    ...
    tarjan_graph_t* new_tarjan_graph
        = (tarjan_graph_t *)calloc(1, sizeof(tarjan_graph_t));
    if (!new_tarjan_graph) {
    ...
    new_tarjan_graph->digraph = (nhqueue_t **)calloc(
        new_tarjan_graph->num_nodes, sizeof(nhqueue_t *));
    for(i=0; i < new_tarjan_graph->num_nodes; i++) {
        new_tarjan_graph->digraph[i] = queue_init();
    }
    new_tarjan_graph->preCount = 0;
    new_tarjan_graph->lowlink  = (int *)calloc(
        new_tarjan_graph->num_nodes, sizeof(int));
    new_tarjan_graph->visited  = (uint8_t *)calloc(
        new_tarjan_graph->num_nodes, sizeof(uint8_t));
