...
/*
  ...
  - array_length -> How long the arrays are. - Approximately
    equal to # of threads in the system
  - cpu_for_thread[i] -> OS thread ID (i) runs on OS CPU
    ID cpu_for_thread[i]
  - utid_for_thread[i] -> OS thread ID (i) maps to user's
    specified thread ID utid_for_thread[i]
 */
...
// Globals
extern cpu_thread_info_t cpu_thread_mapper;
      // ^will hold each thread's cpu id
...
{
    // Assume 1 thread to start, will realloc as additional
    // threads are identified
    cpu_thread_mapper.cpu_for_thread = (int32_t *)calloc(
        1, sizeof(int32_t));
    if (NULL == cpu_thread_mapper.cpu_for_thread) {
        error_print("failed alloc_threadid_stuff calloc of cpu_for_thread");
        return 0;
    }
    cpu_thread_mapper.utid_for_thread = (int32_t *)calloc(
        1, sizeof(int32_t));
...
static inline int realloc_threadid_stuff(void)
{
    ...
    cpu_thread_mapper.cpu_for_thread = (int32_t *)realloc(
    ...
    cpu_thread_mapper.utid_for_thread = (int32_t *)realloc(
    ...
    // (newly created entries in user_threadid to
    //  real_threadid mapper are initialized to -1 to
    //  indicate invalid id's)
    // initialize the new entry
    cpu_thread_mapper.cpu_for_thread[cpulen-1]  = -1;
    cpu_thread_mapper.utid_for_thread[cpulen-1] = -1;

    num_shq_writers = (uint32_t *)realloc(
        num_shq_writers,
    ...
    shq_writers = (uint32_t **)realloc(
        shq_writers, cpulen*sizeof(uint32_t *));
    if (NULL == shq_writers) {
        error_print("failed realloc_threadid_stuff realloc of shq_writers");
        return 0;
    }

    shq_writers[cpulen-1] = (uint32_t *)calloc(
        1, sizeof(uint32_t));
    if (NULL == shq_writers[cpulen-1]) {
...
static inline int realloc_shq_writer(
        uint32_t** shq_writers,
        int i,
        int len)
{
    shq_writers[i] = (uint32_t *)realloc(
        shq_writers[i], len*sizeof(uint32_t));
...
static inline int register_shq_writer(
        uint32_t src_tid,
        uint32_t sub_tid)
{
    uint32_t reg = 0;
    ...
    for (uint32_t i=0; i<num_shq_writers[sub_tid]; i++) {
    ...
    // register that a writer exists for the destination's
    // shared queue
    if (0 == reg) {
...
static inline void report_shq_writers(mimo_t* mimo)
{
    ...
    for (dst=0; dst<work_size; dst++) {
        ...
        // build directed graph for use in detecting
        // the existence of waiting cycles, a
        // pre-condition for processing deadlock.
        // The number of nodes = number of threads, 
        // while the edges = sharedqueue-provided links
        // between threads to be used in this graph
        for (i=0; i<num_shq_writers[dst]; i++) {
            uint32_t* dstId = (uint32_t *)malloc(sizeof(uint32_t));
            *dstId = dst;
            // ^really, just for ensuring heap memory
            // content in queue (not function stack
            // memory content!)
    ...
    fprintf(stderr, "SPECIFIED WS GRAPH HAS AT LEAST"
            " %d-CYCLE(S) IN THE GRAPH...ENABLING"
            " DEADLOCK RECOVERY\n",
            mimo->tg->num_scc);
    // this WS graph will need to initialize failover
    // queues for use when there is deadlock
    graph_has_cycle = 1;
    ...
    j = 0;
    mimo->mgc = (mimo_graph_cycle_t*)calloc(
        mimo->tg->num_scc,
        sizeof(mimo_graph_cycle_t));
    for (index1 = queue_size(mimo->tg->scc_list)-1;
         index1 >= 0;
         index1--)
    {
        nhqueue_t* list = (nhqueue_t *)queue_get_at(
            mimo->tg->scc_list,
            index1);
        if (queue_size(list) > 1)
        {
            mimo->mgc[j].nthreads = queue_size(list);
            mimo->mgc[j].fullshq = (uint8_t *)calloc(
                work_size, sizeof(uint8_t));
            mimo->mgc[j].threadlist = (int *)malloc(
                sizeof(int) * mimo->mgc[j].nthreads);
            for (index2 = mimo->mgc[j].nthreads-1;
                 index2 >= 0;
                 index2--)
            {
                mimo->mgc[j].threadlist[index2]
                    = *(int *)queue_get_at(list, index2);
                mimo->thread_in_cycle[
                    mimo->mgc[j].threadlist[index2] ] = 1;
                // ^Note: we could set this more than
                // once, but it is okay
            }
            j++;
        }
...
// returns 0 if thread_context has not been visited
// returns 1 if it has
static inline int isvisited_utid(int thread_context) {
    for (int i=0; i < cpu_thread_mapper.array_length; i++) {
...
static inline int next_available_utid(void) {
    ...
    for (i=0; i < max_utid_num; i++) {
        ...
        if (0 == found_i) { // 'i' is unique & unused
            return i;
        }
    }
...
static inline void edge_trans(
        mimo_t* mimo,
        ws_proc_instance_t* src, 
        ws_proc_edge_t* edge,
        ws_proc_instance_t* dst, 
        int thread_trans, int thread_context, 
        int twoD_placement)
{
    // We need this to allocate subscribers as either local
    // or external in ws_init_instance_input.
    // The thread_trans and thread_context stuff will never
    // be executed for non-pthreads inits, since the
    // thread_context and thread_trans values should be zero.
    edge->thread_trans = thread_trans;

    // if we have a set thread_context, which could also be 0
    if (-1 != thread_context) {
    ...
    // if tid_assigned is already set, this is a secondary
    // edge - do not create another thread at this point
    ...
    // we indicate that we have visited this kid and
    // assigned it a thread id this is important when making
    // decisions in thread_context section of ws_new_edge
    dst->tid_assigned = 1; 
    ...
    if (cpu_thread_mapper.utid_for_thread[mimo->thread_id] == -1) {
        cpu_thread_mapper.utid_for_thread[mimo->thread_id] = dst->thread_id;
    }
}
...
static inline uint32_t get_thread_realid(int user_threadid) {
    int i = 0;
    // default all unmapped user_threadid to a
    // real_threadid of '0'
    uint32_t retval = 0;
...
// just for debugging purposes...prints out the assigned
// values in user-real id mapper
static inline void display_assigned_utr_values(void) {
    for (int i=0; i < cpu_thread_mapper.array_length; i++) {
        if (cpu_thread_mapper.cpu_for_thread[i] != -1) {
            fprintf(stderr,
                "user_tid %d maps to "
                "real_tid %d on core %d\n",
...
static inline void offset_cpus_and_uid_assignments(
...
// this is really only used when we need to remove userid = 0
// from the list of valid utr's as parsing the entire graph
// indicated that it was not used
static inline void rearrange_and_remove_invalid_userid(
        mimo_t* mimo)
{
...
static inline void set_cpu_for_thread(
        uint32_t mimotid,
        uint32_t thread_context)
{
    if (mimotid <= 1) {
        // a single value is already in a ascending order
        cpu_thread_mapper.utid_for_thread[mimotid]
            = thread_context;
        return;
    }
    ...
    int i;
    for (i=0; i<mimotid; i++) {
        if (thread_context < cpu_thread_mapper.utid_for_thread[i])
            break;
    }

    // move contents of utid_for_thread one step ahead
    for(int j=mimotid; j>i; j--) {
        cpu_thread_mapper.utid_for_thread[j]
            = cpu_thread_mapper.utid_for_thread[j-1];
    }
...
static inline void set_pinst_tid(
        mimo_t* mimo,
...
static inline void ws_recast_proc_ids(mimo_t* mimo) {
    ...
    for (cursor = mimo->proc_instance_head;
         ...
         cursor = cursor->next)
    {
        cursor->thread_id = get_thread_realid(
            cursor->thread_id);
        cursor->doutput.local_jobq
            = mimo->jobq[cursor->thread_id];
        cursor->doutput.local_job_freeq
            = mimo->jobq_freeq[cursor->thread_id];
    }
}
...
static inline void rebase_threads_to_cpu(mimo_t* mimo)
{
    ...
    if (0 == nrank) {
        offset_cpus_and_uid_assignments(
            mimo->thread_id+1,
            offset_value);
        display_assigned_utr_values();
    }

    // need to sync up to ensure that no thread speeds off
    // before nrank0 gets a chance to set the global
...
// end
    ...
    status_print("Thread %d (User thread %d) set to run "
                 "on CPU: %d",
                 nrank,
                 cpu_thread_mapper.utid_for_thread[nrank],
                 my_id);
    if (0 == nrank)
...
#ifndef WS_PTHREADS
static inline void rebase_threads_to_cpu(mimo_t * mimo) { 
    // need to set the local_jobq and local_job_freeq
