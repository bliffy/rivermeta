...
{
     mimo->shared_jobq = (shared_queue_t**)calloc(
          work_size, sizeof(shared_queue_t*));
     if (!mimo->shared_jobq) {
     ...
     // XXX: probably not the most logical place for this
     // allocation of memory, but this'll do
     mimo->thread_in_cycle = (uint8_t*)calloc(
...
// only need to init failover q if at least one cycle
// exist in mimo
static inline int mimo_init_failoverq(mimo_t* mimo)
{
     if (mimo->tg->num_scc) {
          mimo->failoverq = (failoverqueue_t**)calloc(
               work_size, sizeof(failoverqueue_t*));
          if (!mimo->failoverq) {
...
static inline int mimo_cycle_deadlock_exist(mimo_t* mimo) {
     // XXX: should be mostly correct and sufficient...
     // need synchronization mechanism to guarantee
     // correctness
     int i, j, ret = 0;
     for (i=0; i<mimo->tg->num_scc; i++) {
          ret = 1; // assume that a cycle exist on an entry
...
static inline void* mimo_run_threaded_graph(
          mimo_work_order_t* arglist)
{
     ...
     // On short runs, we get a race-ahead condition here,
     // which allowed the first-arrival to create, use, and
     // destroy a shared hash table, before other nodes are
     // done with it.
     if (mimo->verbose) {
          fprintf(stderr,"REACHED EXECUTE-GRAPH BARRIER on %d\n",nrank);
     }

     // setup spinning_on_jobs for use in avoiding deadlock
     // that could arise when a kid spits out several
     // metadata that are aimed at the shared queues of
     // other threads (shared_queues have fixed size and use
     // blocking writes)
     if (0 == nrank) {
          // assumes that work_size has been set ...
          // in this case, it should have been done by
          // mimo_compile_graph in the waterslide-parallel.c
          // code prior to calling this function,
          // mimo_run_threaded_graph
     ...
     while (!do_exit) {
          if (!mimo_run_graph(mimo)) {
               ...
               // must use the same lock here as setup_exit()
               WS_MUTEX_LOCK(&exit_lock);
               num_src_threads--; // remove this thread from
                              // count of source candidates
               if (0 == num_src_threads) {
                    if (!do_exit) {
                         do_exit = 1; // eventually, all
                              // threads will see this value
                    }
               }
     ...
     // unfortunately, with 'signal' semantics, not all
     // threads receive a SIGXXXX (e.g. SIGINT) at the same
     // time, thus we need an indicator of when all threads
     // have stopped their (possibly) respective sources
     WS_MUTEX_LOCK(&(mimo->lock));
     ready_to_flush++;
     WS_MUTEX_UNLOCK(&(mimo->lock));

     // execute jobs from external queue until all threads
     // have cutoff their proc_sources (source kids)
     if (work_size > 1) {
          do {
     ...
     // ensure that no thread hangs because it cannot
     // enqueue/write metadata in the shared (external)
     // queue of a thread who may be in a barrier_wait
     // in the function "ws_empty_allqueues_synchronously"
     int toggle = 0, job_cnt = 0;
     while( 1 ) {
          if( 0 == spinning_on_jobs ) {
               // all threads have successfully pushed out
               // their metadata without being blocked out
               // because of limited size in shared queues
               break;
          }
     ...
     // Now, walk through local+external jobs of threads,
     // one at a time.  In this process, other threads must
     // continue to spin on their external queues to avoid
     // deadlock. We repeat this process until no jobs are
     // queue'd.
     ws_empty_allqueues_synchronously(mimo);
          
     // At this point, all queues are empty of jobs, but
     // there could still be jobs in the state tables of
     // kids that will be flushed out
     if (0 == nrank) {
          // initialized to zero; when set, this indicates
          // that the flush routine did not execute as
          // expected on a set of kids
          flushes_aborted = 0;

          // set all outputs to thread 0's local queue. we
          // will henceforth process the flushes as a serial
          // process
          //
          // step 1. set all kid's output to go to thread 
          // 0's local jobq
               ...
               cursor = cursor->next)
          {
               ...
               // we are done with shared_jobq_array, so
               // nullify them (seg fault will be an obvious
               // sign of some jobs not going to the local
               // jobq[nrank] as intended)
               cursor->doutput.shared_jobq_array = NULL;

               if (!cursor->output_type_list.outtype_q)
                    continue;

               for (qcursor = cursor->output_type_list.outtype_q->head;
                    qcursor != NULL;
          ...
          if (!mimo_flush_graph(mimo)) {
               // the flush has gone wrong (e.g., badi
               // wsd_flush alloc)
               flushes_aborted = 1;
          }
     }
     ...
     // Free memory, destroy barriers, locks, etc.
     if (!ws_cleanup(mimo)) {
          exit(0);
     }
...
static inline int mimo_create_threads(mimo_t* mimo)
{
     ...
     // Create threads, have them initialize fields, and
     // then conditionally wait until the master signals
     // them to get to work (mimo_run_graph).
     // This is necessary to allow us serialize the critical
     // sections first. Also, set up mimo for the threads
     // in global memory array.
     ...
     // main() thread will also execute
     // mimo_threaded_run_graph, but outside this for-loop
     // (in driver code)
     arglist[0]->rank = 0;
     arglist[0]->mimo = mimo;
     for (int i = 1; i < work_size; i++) {
          ...
          pthread_create(
               thread[i],
               &attr,
               (void *(*)(void *))mimo_run_threaded_graph,
               arglist[i]);
     }
