...
static inline wsfree_list_t * wsfree_list_init(
...
static inline void wsfree_list_element_destroy(
          wsfree_list_t * fl)
{
...
static inline void* wsfree_list_alloc(wsfree_list_t * fl)
{
     void * node = wsstack_remove(fl->stack);
     if (NULL == node) {
          if (fl->max_allocated == 0 
              || fl->allocated_count < fl->max_allocated)
          {
...
static inline int wsfree_list_free(
          wsfree_list_t * fl,
          void * data)
{ return wsstack_add(fl->stack, data); }


static inline unsigned int wsfree_list_size(
          wsfree_list_t * fl)
{ return wsstack_size(fl->stack); }


static inline unsigned int wsfree_list_allocated(
          wsfree_list_t * fl)
...
static inline wsfree_list_local_cache_t * wsfree_list_get_cache(
          wsfree_list_t * fl)
{
     ...
     if (tls->cache) {
          ...
          if (NULL == node) {
               ...
               if (fl->max_allocated == 0 
                   || fl->allocated_count < fl->max_allocated)
               {
                    node = (wsfree_list_node_t*)fl->allocator(
                         fl->allocator_data);
                    if (NULL != node)
                         fl->allocated_count++;
               }
          ...
          void * data)
{
     ...
     if (cnt == (BLOCK_SIZE * 2)) {
          ...
          wsfree_list_node_t * toglobal = 
               node->block_tail->next;
          node->block_tail->next = NULL;
          WS_SPINLOCK_LOCK(&fl->lock);
...
static inline void* wsfree_list_alloc(wsfree_list_t * fl)
{
     ...
     if (NULL == element) {
          if (fl->max_allocated == 0
              || cache->allocated_count < fl->max_allocated)
          {
...
static inline wsfree_list_node_t * wsfree_list_swap(
          wsfree_list_node_t *volatile *addr,
