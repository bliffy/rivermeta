// defines the basic data type resolver structure..
...
typedef uint8_t * (*wsdatatype_serialize)(
     wsdata_t *,
     int *);

typedef enum {
     WS_PRINTTYPE_TEXT    = 1,
     WS_PRINTTYPE_BINARY  = 2,
     WS_PRINTTYPE_XML     = 3,
     WS_PRINTTYPE_HTML    = 4
} ws_printtype_t;
...
typedef wsdata_t * (*wsdatatype_subelement)(
     wsdata_t * /*parent*/,
     wsdata_t * /*dst*/,
     void * /*aux*/);
...
static inline int wsdatatype_register_subelement(
          wsdatatype_t * dtype,
          void * type_table,
          const char * label,
          ...
          int offset)
{
     ...
     wssubelement_t * sub =
          & dtype->subelements[dtype->num_subelements];
...
static inline int wsdatatype_register_subelement_func(
          wsdatatype_t * dtype,
          void * type_table,
          const char * label,
          const char * dtypename,
...
static inline wssubelement_t * wsdatatype_find_subelement(
          wsdatatype_t * dtype,
          wslabel_t * label)
{
     wssubelement_t * sub;
...
static inline int wsdtype_build_subelement_list(
          wsdatatype_t * dtype,
          wslabel_set_t * lset,
          wsdtype_sub_list_t * slist)
{
     ...
     for (int i = 0; i < lset->len; i++) {
          sub = wsdatatype_find_subelement(
               dtype, lset->labels[i]);
          if (sub) {
...
static inline void wsdata_core_init(
          wsdata_t * wsdata,
          wsdatatype_t * dtype)
{
     wsdata->references       = 0;
...
// allocate & init new data from datatype -- useful for
// every module
static inline wsdata_t * wsdata_alloc(wsdatatype_t *);

static inline wsdata_t * wsdata_get_subelement(
          wsdata_t * wsd,
          wssubelement_t * sub)
{
     if (!sub || !wsd)
     ...
     if (!sub->dtype) {
          if (sub->sub_func) {
               wsdata_t * out = sub->sub_func(
                    wsd, NULL, sub->aux_data);
               if (out && sub->label 
                   && !wsdata_check_label(out, sub->label)) {
...
// allowed for a centralized control of types a new data
// type could be loaded from a module/shared object
...
static inline wsdata_t * wsdata_ptr(wsdata_t * ref) {
     if (!ref)
          return NULL;
     wsdata_t * newdata = (wsdata_t*) wsfree_list_alloc(
          ref->dtype->freeq_ptr);
     if (NULL == newdata)
          return NULL;
