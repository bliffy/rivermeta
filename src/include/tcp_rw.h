...
static int establish_connection(tcp_comm_t* comm)
{
     int sd, ret;
     struct sockaddr_in sa_in;
     socklen_t socklen;
...
inline static int check_connection_status(tcp_comm_t* comm)
{
     int ret, sd;
     if (comm->server_socket < 0) {
          /* make sure we are connected */
          if (comm->fds[0] < 0 
              && comm->last_connect_try+1 < time(NULL))
          {
               comm->fds[0] = establish_connection(comm);
               if (comm->fds[0] >= 0)
                    comm->num_clients++;
               else
...
static int comm_init(tcp_comm_t* comm)
{
     if (NULL == comm->hostname) {
          /* Run as server */
          comm->server_socket = create_listen_socket(
               comm->hostname,
               comm->port);
          if (comm->server_socket < 0)
               return -1;
...
static int comm_destroy(tcp_comm_t* comm)
{
     if (comm->server_socket >= 0)
          close(comm->server_socket);
     for (int i=0; i<comm->num_clients; ++i)
          close(comm->fds[i]);
     return 0;
}
...
tcp_throw_t* tcp_throw_init(
          const char* hostname,
          int port,
          int blocking)
{
     int ret;
     tcp_throw_t* thrower = (tcp_throw_t *)calloc(
          sizeof(tcp_throw_t), 1);
     if (!thrower) {
...
int tcp_throw_destroy(tcp_throw_t* thrower)
{
     /* flush out partially sent messages so receiver doesn't timeout */
     for (int i=0; i<thrower->base.num_clients; ++i) {
...
int tcp_throw_data(tcp_throw_t* thrower, wsdata_t* data)
{
     int ret, j;
...
tcp_catch_t* tcp_catch_init(
          const char* hostname,
          int port,
          int blocking)
{
     int ret;
     tcp_catch_t* catcher = (tcp_catch_t *)calloc(
          sizeof(tcp_catch_t), 1);
     if (!catcher) {
     ...
     if ( (ret = comm_init(&catcher->base)) < 0 ) {
          free(catcher);
          return NULL;
     }
     return catcher;
...
int tcp_catch_destroy(tcp_catch_t* catcher)
{
          ...
          ws_doutput_t* dout)
{
     ...
     if (ret < 0)
          return 0;
     for (int i=0; i<catcher->base.num_clients; ++i) {
          ret = tcp_catch_recv_msg(
               catcher->base.fds[i], outtype, data, dout);
          if (ret < 0) {
               close(catcher->base.fds[i]);
               catcher->base.fds[i] = -1;
               for (j=i; j<catcher->base.num_clients-1; j++) {
                    catcher->base.fds[j]
                         = catcher->base.fds[j + 1];
               }
               i--;
